...
lob_size <- lob_raw %>% 
  ...
  filter(year==2018|year==2012) ###### I think we need to fix this chunk. I looked at the lob size table and it wasn't returning any values with 2018 only ones with 2012.  Also think it would be good to label this lob_size_2018 and the next lob_size_2012
...
```{r}
### MPA = Naples and Isla Vista
### nonMPA = Arroyo Quemado, Mohawk, Carpinteria

# Create data frames for statistical analysis.

# Mean size of lobsters in 2012

lob_mean_size_2012 <- lob_raw %>% 
  filter(year == 2012) %>%
  filter(!is.na(size_mm)) %>% 
  group_by(site) %>% 
  summarize(
...
# MPA and non MPA mean sizes for 2012
mpa_mean_size_2012 <- lob_mean_size_2012 %>% 
  filter(site %in% c("IVEE","NAPL" ))

non_mpa_mean_size_2012 <- lob_mean_size_2012 %>% 
  filter(site %in% c("AQUE", "CARP", "MOHK"))

# MPA and non MPA mean sizes for 2012

mpa_mean_size_2018 <- lob_mean_size_2018 %>% 
  filter(site %in% c("IVEE","NAPL" ))

non_mpa_mean_size_2018 <- lob_mean_size_2018 %>% 
  filter(site %in% c("AQUE", "CARP", "MOHK"))
```
